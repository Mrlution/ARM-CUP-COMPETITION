...
wire		[7:0]									 i_h_left_pixel_ret;
wire       [7:0]                                    i_h_right_pixel_ret; 
wire		[7:0]									 i_v_up_pixel_ret;
wire       [7:0]                                    i_v_down_pixel_ret;

assign {i_h_left_pixel_ret, i_h_right_pixel_ret, i_v_up_pixel_ret, i_v_down_pixel_ret}  =    i_ret_data;
...
always @(posedge pixelclk or negedge reset_n) begin
  if(!reset_n) begin 
    ...
    FRAME1:begin
     ...
     if(h_pedge_r0 && h_pedge_cnt == 1) hcount_l1_r <= (h_raddr + i_h_left_pixel_ret);
   
     if(h_nedge) h_nedge_cnt <= h_nedge_cnt +1;
     if(h_nedge_r0 && h_nedge_cnt == 1) hcount_r1_r <= (h_raddr - i_h_right_pixel_ret);
      
     if(v_pedge) v_pedge_cnt <= v_pedge_cnt +1;
     if(v_pedge_r0 && v_pedge_cnt == 1) vcount_l1_r <= (v_raddr + i_v_up_pixel_ret);
   
     if(v_nedge) v_nedge_cnt <= v_nedge_cnt +1;
     if(v_nedge_r0 && v_nedge_cnt == 1) vcount_r1_r <= (v_raddr - i_v_down_pixel_ret);
	  end
	  else begin
